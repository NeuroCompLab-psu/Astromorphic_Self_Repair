function PR_seq = PR_simulation(totalTime, dt, synapse_per_neuron, faulty_percentage, ave_PR)
    del_t = dt;                   %1 ms
%     totalTime = 400;                 %400 s
    ...
    num_faulty_synapse = round(synapse_per_neuron * faulty_percentage);

    PR_N1 = zeros(size(totalStep,2), synapse_per_neuron);
    PR_N1(1,:) = ave_PR*ones(1);
    PR_N2 = zeros(size(totalStep,2), synapse_per_neuron);
    
    while true
        PR_N2(1,:) = rand(1,synapse_per_neuron);
        PR_N2(1,:) = PR_N2(1,:) * ave_PR * synapse_per_neuron / sum(PR_N2(1,:));
        validPR = isempty(find((PR_N2(1,:) > 1),1));
        if validPR
            break
        end
    end
    ...
    spikesPerS = 10;      % 10 spikes per second, on average
    ...
    for j = 1:synapse_per_neuron
        ...
        spikes_N1(:,j) = (spikesPerS*del_t) > vt;
        vt = rand(size(totalStep));
        spikes_N2(:,j) = (spikesPerS*del_t) > vt;
    end
